"""
Carpet GOD List to Ticket Data
(c) 2024 Woven & Woods
...
def process_data(gl_data, ticket_data):
    ...
    for _, row in df.iterrows():
        ...
        new_data = pd.DataFrame({
            ...
            "Widths": [generate_widths(row["Width(s) (M)"])],
...
def generate_widths(widths):
    ...
    if not isinstance(widths, str):
        return f"Width: {str(widths)}M"
    for char in widths.replace("&", ","):
        if char in "0123456789,.":
            clean_widths += char
    output = "Widths: "
    widths_list = clean_widths.split(",")
    for width in widths_list:
        output = output + width + "M, "
    output = output[:-2]
    return "".join(output[::-1].replace(",", "& ", 1)[::-1])
